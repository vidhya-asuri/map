...
function initialize() {
    ...
    function nearbyResults() {
        ...
        self.displayBakeries = function(results) {
            self.allResults.bakeries = results;
            for (var b in self.allResults.bakeries) {
                console.log(self.allResults.bakeries[b]);
                console.log("\n");
            }
        };
        self.displayBookstores = function(results) {
            self.allResults.bookstores = results;
            for (var s in self.allResults.bookstores) {
                console.log(self.allResults.bookstores[s]);
                console.log("\n");
            }
        };
        self.displayParking = function(results) {
            self.allResults.parkingLots = results;
            for (var lot in self.allResults.parkingLots) {
                console.log(self.allResults.parkingLots[lot]);
                console.log("\n");
            }
        };
    ...
    function getPlaceID(results, status) {
        ...
        function markBakeries(results, status) {
            if (status === google.maps.places.PlacesServiceStatus.OK) {
                resultsObj.displayBakeries(results);
...
function displayResults(results) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        ulElement.appendChild(resultListItem);
        var listItem_P_Element = document.createElement("p");
        listItem_P_Element.innerText = results[i].name;
        resultListItem.appendChild(listItem_P_Element);
    }
    // add the newly created element and its content into the DOM 
    var resultsDiv = document.getElementById("results");
...
function pointsOfInterestModel(results) {
    var self = this;
    // Non-editable catalog data - would come from the server
    // results from Google nearby search are passed in as the results variable.
